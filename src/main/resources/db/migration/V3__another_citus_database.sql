CREATE DATABASE another_citus_database;
SELECT DBLINK_EXEC('dbname=another_citus_database user=postgres', $remote$
CREATE EXTENSION citus;
$remote$);
-- Creates database on every worker
SELECT run_command_on_workers($cmd$CREATE DATABASE another_citus_database;$cmd$);
...
SELECT DBLINK_EXEC(FORMAT('host=%s dbname=another_citus_database user=postgres', node_name), $remote$
CREATE EXTENSION citus;
$remote$)
FROM citus_workers;
...
SELECT DBLINK_EXEC('dbname=another_citus_database user=postgres', format($remote$
